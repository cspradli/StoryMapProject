...
L.GeoCSV=L.GeoJSON.extend({
    options:{
        ...
        fieldSeparator:",",
        lineSeparator:"\n",
        deleteDobleQuotes:true,firstLineTitles:false
    }
    ,_propertiesNames:[],
...
addData:function(data){
    if(typeof data==="string"){
        ...
        if(this.options.firstLineTitles){
            ...
            for(var i=0;i<titulos.length;i++){
                var prop=titulos[i].toLowerCase().replace(/[^\w ]+/g,"").replace(/ +/g,"_");if(prop==""||prop=="_"||this._propertiesNames.indexOf(prop)>=0)prop="prop-"+i;this._propertiesNames[i]=prop}data=this._csv2json(data)}
                L.GeoJSON.prototype.addData.call(this,data)},
                getPropertyName:function(title){var pos=this.options.titles.indexOf(title),
                    prop="";
                    ...
                    return title},_deleteDobleQuotes:function(cadena){
